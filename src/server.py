...
class Server:
    def __init__(self, ip, port, args):
        self.storage_dir = (args.storage
                            if args.storage[-1] == '/' else args.storage + '/')
        self.ip = ip
        self.port = port
    ...
    def start(self):
        ...
        Si recibe un request de UPLOAD, lanza un thread
        UploadHandler para manejarlo. Este thread le envia
        al cliente su puerto para comunicarse.

        Si recibe un request DOWNLOAD, lanza un thread
        DownloadHandler para manejarlo. Este thread le envia
        al cliente su puerto para comunicarse.

        Cada cierto tiempo se fija si algún thread termino
        y en tal caso lo joinea.
        """
            ...
            try:
                ...
                if msg.message_type == MessageType.INSTRUCTION:
                    ...
                    if address not in self.port_by_address.keys():
                        handler = HandleFactory.create_handle(
                            self.ip, msg.data, address,
                    ...
                    if msg.data == UPLOAD:
                        port_msg = Message(
                            MessageType.PORT, 0,
                            str(self.port_by_address[address]), "")
